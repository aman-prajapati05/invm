...
type PODetails = {
  poNumber: string;
  poDate: string;       // Stored as Date in DB (BSON Date). Typing can remain string for API shape.
  deliveryDate: string;
  deliveredTo: string;
  paymentTerms: string;
  gstNo: string;
  ...
  switch (method) {
    case 'GET': {
      try {
        ...
        // Date filtering – field is a BSON Date in DB, so we can match directly
        if (startDate || endDate) {
          match['data.poDetails.poDate'] = {};
          if (startDate) match['data.poDetails.poDate'].$gte = new Date(startDate as string);
          if (endDate) match['data.poDetails.poDate'].$lte = new Date(endDate as string);
        }
        ...
        const orders = await collection
          .aggregate([
            { $match: match },
            // ✅ Sort directly by BSON Date (newest first). Add _id tiebreaker for stability.
            ...
            { $limit: limitNum },
            {
              $project: {
                ...
                location: { $ifNull: ['$data.poDetails.location', '$poDetails.deliveredTo'] },
    ...
    case 'POST': {
      ...
      if (!po_number || !source || !poDetails || !items) {
        return res
          .status(400)
          .json({ message: 'po_number, source, poDetails, and items are required' });
      }
